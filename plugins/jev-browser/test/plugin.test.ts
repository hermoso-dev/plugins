...
test("registers the complete Jev Browser surface", () => {
	...
	assert.deepEqual(rules, ["cline-jev-browser-safety"]);
});
...
test("loads bounded config values", () => {
	const directory = mkdtempSync(join(tmpdir(), "cline-jev-browser-"));
...
test("uses complete defaults when configuration is omitted", () => {
	const config = readConfig(
		join(tmpdir(), "missing-cline-jev-browser.config.json"),
	);
