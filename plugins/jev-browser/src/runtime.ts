...
export class JevBrowserManager {
	...
	async run(input: BrowserRunInput, context: ToolContext) {
		...
		try {
			...
			const initial = await this.screenshot({ label: "jev-initial" }, context);
	...
	private async stopByKey(key: string) {
		...
		if (!session)
			return {
				active: false,
				message: "No browser is active for this Cline session.",
			};
...
function screenshotResult(
	...
	return {
		...
		result: [
			...
			{ type: "image", data: image.toString("base64"), mediaType: "image/png" },
		],
	};
}
...
function assertUrlAllowed(url: string, config: JevBrowserConfig) {
	if (!isUrlAllowed(url, config.allowedOrigins)) {
		throw new Error(
			`Navigation blocked by cline-jev-browser.config.json: ${url}`,
		);
	}
}
