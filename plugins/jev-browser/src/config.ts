...
export const CONFIG_PATH =
	process.env.CLINE_JEV_BROWSER_CONFIG?.trim() ||
	join(homedir(), ".cline", "plugins", "cline-jev-browser.config.json");
...
const DEFAULT_CONFIG: JevBrowserConfig = {
	...
	outputDir: join(homedir(), ".cline", "data", "jev-browser"),
...
export function readConfigFile(path = CONFIG_PATH): Record<string, unknown> {
	try {
		...
		throw new Error(
			"Cannot read cline-jev-browser.config.json. Check permissions and JSON syntax.",
		);
	}
}
